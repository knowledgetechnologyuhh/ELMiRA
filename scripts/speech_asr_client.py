...
# Usage: rosrun nico_demo speech_asr_client.py [PARAMS]
...
class SpeechASRClient(multi_action_client.MultiActionClient):
	...
	def __init__(self, ns='speech_asr', verbose=False):
		super().__init__(ns=ns, ActionSpec=nico_demo.msg.PerformASRAction, active_cb=self.active_cb, done_cb=self.done_cb, feedback_cb=self.feedback_cb)
	...
	def perform_asr(self, detect_start=True, detect_stop=True, start_timeout=0.0, min_duration=3.0, max_duration=30.0, min_period=3.0, live_text=False, done_timeout=None) -> str:
		goal = nico_demo.msg.PerformASRGoal(detect_start=detect_start, detect_stop=detect_stop, start_timeout=start_timeout, min_duration=min_duration, max_duration=max_duration, min_period=min_period, live_text=live_text)
		action_goal = self.send_goal(goal=goal, wait_done=True, done_timeout=done_timeout, auto_cancel=True, wait_cancel=True)
		result: nico_demo.msg.PerformASRResult = action_goal.get_result()  # noqa
	...
	def feedback_cb(self, goal_id: str, feedback: nico_demo.msg.PerformASRFeedback):
		if self.verbose:
